...
_config = {
    "symbol": "ETH", "coin": 'ETHUSDT', "arttir": 30,
...
start = datetime.strptime('2022-11-20 00:00:00', '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
_son = bitis_gunu_truncate_min_precision(start, 30)

while window_end < _son:

    sqlite_service.mum_datasi_yukle(
        "30m", binance_serve, window_end - timedelta(days=10), window_end
    )
