...
class RsiEmaStrategy:
    def __init__(self, config):
        ...
        self.rsi_bounding_limit = config.get("rsi_bpunding_limit")
        self.ema_bounding_limit = config.get("ema_bpunding_limit")
                ...
                if self.momentum_trend_rsi == Karar.satis:
                    if self.rsi_emasi_karar == Karar.satis:
                        if self.rsi_smasi_trend == Karar.alis \
                                and self.rsi_value < 100 - self.rsi_bounding_limit:
                ...
                if self.momentum_trend_rsi == Karar.alis:
                    if self.rsi_emasi_karar == Karar.alis:
                        if self.rsi_smasi_trend == Karar.satis \
                                and self.rsi_value > self.rsi_bounding_limit:
