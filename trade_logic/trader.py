...
class Trader(TraderBase):
    def karar_calis(self):
        if self.heikinashi_karar == Karar.alis and self.rsi_strategy.karar == Karar.alis:
            self.karar = Karar.alis
        if self.heikinashi_karar == Karar.satis and self.rsi_strategy.karar == Karar.satis:
    ...
    def rsi_cikis_veya_donus(self):
        if self.pozisyon != Pozisyon.notr:
            if self.rsi_strategy.karar == Karar.cikis:
                ...
                self.super_trend_strategy.reset_super_trend()
            # if self.rsi_strategy.rsi_smasi_trend != Karar.notr:
            #     if self.rsi_strategy.rsi_smasi_trend.value != self.pozisyon.value:
            #         self.karar = Karar.cikis
            #         self.super_trend_strategy.reset_super_trend()
    ...
    def heikinashi_kontrol(self):
        series = heikinashiye_cevir(self.series)
